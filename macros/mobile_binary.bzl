...
# Should be used for native, WASM and iOS targets generated by the mobile_binary macro

def create_transitioned_binary_rule(transition):
    def _impl(ctx):
        # We need to copy the executable because bazel doesn't allow
        # providing an executable not created by the rule
        executable_src = ctx.executable.actual_binary
        ...
        runfiles = ctx.attr.actual_binary[DefaultInfo].default_runfiles
        providers = [DefaultInfo(runfiles = runfiles, executable = executable_dst)]

        if IosApplicationBundleInfo in ctx.attr.actual_binary:
            providers.append(ctx.attr.actual_binary[IosApplicationBundleInfo])

        if AppleBundleInfo in ctx.attr.actual_binary:
            providers.append(ctx.attr.actual_binary[AppleBundleInfo])
    ...
    return rule(
        ...
        attrs = {
            "actual_binary": attr.label(cfg = "target", executable = True),
        },
        executable = True,
    )

# Should be used for Android and WASM (-all-deps target) targets generated by the mobile_binary macro
def create_transitioned_non_binary_rule(transition):
    def _impl(ctx):
        providers = [ctx.attr.actual_binary[DefaultInfo]]
        ...
        if ApkInfo in ctx.attr.actual_binary:
            providers.append(ctx.attr.actual_binary[ApkInfo])
    ...
    return rule(
        ...
        implementation = _impl,
        attrs = {
            "actual_binary": attr.label(cfg = "target"),
        },
    )
